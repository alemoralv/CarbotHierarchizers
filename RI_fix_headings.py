...
Adapted specifically for the RI (Régimen de Inversión) document, which
differs from the CUF document in several structural ways:
  - Hierarchy entries have descriptive suffixes after em-dashes
    (e.g. "SEGUNDA — Definiciones") that the MD text doesn't have.
  - Roman numerals appear with inline definition text (no dash separator).
  - Non-standard numeral extensions like "XXXa.", "XXXb.".
  - Lettered sub-items "a)", "b)" appear with inline text.
  - CONSIDERANDO (singular) vs CONSIDERANDOS (plural).
  - Title uses "SIEFORES" abbreviation vs full expansion in MD.
  - Ordinal dispositions (PRIMERA, SEGUNDA, ...) instead of "Artículo N".
...
Usage:
    python RI_fix_headings.py inputpdfs/RI.md --hierarchy inputpdfs/RI_hierarchy.md
    python RI_fix_headings.py inputpdfs/RI.md                  # auto-discover hierarchy
    python RI_fix_headings.py inputpdfs/                       # batch mode
    python RI_fix_headings.py inputpdfs/RI.md --inplace
    python RI_fix_headings.py inputpdfs/RI.md --threshold 0.90
"""
...
def _compute_prefix_norm(norm_text: str) -> Optional[str]:
    """Extract prefix before em-dash/en-dash separator.

    Hierarchy entries like 'segunda — definiciones' → prefix 'segunda'.
    Returns None if there is no dash separator or prefix equals full text.
    """
    for sep in (" \u2014 ", " \u2013 ", " -- "):
        if sep in norm_text:
            prefix = norm_text.split(sep, 1)[0].strip()
...
def _try_match(
    ...
    *,
    ...
    Pass 1  — exact normalised match (full scan).
    Pass 1b — exact abbreviated match (windowed, if check_abbrev).
    Pass 1c — exact prefix match (windowed; hierarchy text before em-dash).
    ...
    # --- pass 1: exact on norm_text (full scan) ---
    ...
    if check_abbrev:
        ...
        for i in range(start, end_abbrev):
            ...
            # abbreviated matches from jumping across ANEXO/CAPÍTULO sections
    ...
    # --- pass 1c: exact on prefix before em-dash (full scan) ---
    # Full scan is safe because prefix matches require md_norm == prefix,
    # which is specific enough to avoid false positives.
    for i in range(start, h_len):
...
def _is_title_match(md_norm: str, title_norm: str, threshold: float) -> bool:
    ...
    Handles the SIEFORES abbreviation: the hierarchy uses 'siefores' but
    the MD has the full expansion 'sociedades de inversion especializadas
    de fondos para el retiro'.  Also handles trailing footnote markers.
    """
    ...
    # Handle SIEFORES ↔ full expansion
    _SIEFORES_ABBREV = "siefores"
    _SIEFORES_EXPANSION = (
        "sociedades de inversion especializadas de fondos para el retiro"
    )
    if _SIEFORES_ABBREV in title_norm:
        expanded = title_norm.replace(_SIEFORES_ABBREV, _SIEFORES_EXPANSION)
        if md_norm == expanded:
            return True
        if fuzzy_ratio(md_norm, expanded) >= threshold:
            return True
        # Prefix check: if they share a long common prefix, consider match
        if md_norm.startswith(expanded[:80]) and len(expanded) > 80:
            return True
...
def _try_plain_text_match(
    ...
    # Handles "PRIMERA.- text", "I.- text", "a).- text", etc.
...
def print_report(
    ...
    fuzzy_matches   = [m for m in matches if m.match_type in ("fuzzy", "fuzzy-prefix")]
    ...
    for m in matches:
        ...
        if m.match_type in ("fuzzy", "fuzzy-prefix"):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fix heading levels in MinerU-generated Markdown (RI document) using a hierarchy reference."
    )
    parser.add_argument(
        "target", type=Path,
